...
CREATE POLICY \"Allow authenticated users to select supplier_transactions\"
  ON supplier_transactions
...
-- Policies for purchase_itemsCREATE POLICY \"Allow authenticated users to insert purchase_items\"
  ON purchase_items
  FOR INSERT TO authenticated
  WITH CHECK (true);
